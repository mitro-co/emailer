...
class InviteNewUserTest(unittest.TestCase):
    def test_simple(self):
        ...
        for match in HREF_RE.finditer(html):
            ...
            self.assertEquals('https://mitro.myriadmobile.com/install.html', url)
        ...
        count = 0
        TEXT_RE = re.compile('(https://mitro.myriadmobile.com/[^\s]+)')
        for match in TEXT_RE.finditer(text):
            count += 1
            quoted = urllib.urlencode({'u': to_address, 'p': password})
            self.assertEquals('https://mitro.myriadmobile.com/install.html#' + quoted,
                match.group(1))
...
class VerifyDeviceTest(unittest.TestCase):
    def test_simple(self):
        ...
        html = message.get_payload()[1].get_payload(decode=True).decode('utf-8')
        self.assertTrue('https://mitro.myriadmobile.com/mitro-core/user/VerifyDevice?' in html)
...
class SendAddressVerificationTest(unittest.TestCase):
    def test_simple(self):
        ...
        for match in HREF_RE.finditer(html):
            count += 1
            url, hash = match.group(1).split('?')
            self.assertEquals('https://mitro.myriadmobile.com/mitro-core/user/VerifyAccount', url)
